            ...
            <Modal show={show} onHide={handleClose}>
                ...
                <Modal.Body className="bg-main-900 text-text">
                    ...
                    <div className="overflow-y-scroll h-48">
                        {fetchedWorkouts?.filter(workout => workout.name.toLowerCase().includes(search.toLowerCase())).map((workout, idx) => {
                            return <div onClick={() => selectWorkout(workout)} className={"py-2 text-text-primary " + (selectedWorkouts.find(selectedWorkout => selectedWorkout.name === workout.name) ? "border-s-accent-900 border-s-4" : "")} key={idx}>{workout.name}</div>
                        })}
                    </div>
                </Modal.Body>
